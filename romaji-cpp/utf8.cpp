...
const char *next_utf8_char(const char *input)
{
	...
	if(head & 0x80) {
		...
		while((head <<= 1) & 0x80) {
			if(((*input) & 0xc0) != 0x80) {
                return nullptr; // Trailing byte does not start with the bits '10'
			}
			++input;
			++n;
		}
		//if(n < 0) throw utf8_encoding_error("too few bits set in leading byte (only 1)");
        if (n < 0) return nullptr;  // too few bits set in leading byte (only 1)
		//if(n > 5) throw utf8_encoding_error("too many bits set in leading byte (more than 6)");
        if (n > 5) return nullptr;  // too many bits set in leading byte (more than 6)
	...
	if(size == 1) {
		...
		*/
		...
		if(out < lower_limit) {
            ...
            return nullptr; // shortest possible multi-byte sequence was not used
		}
